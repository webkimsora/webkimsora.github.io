...
//event02Flag = false;
  ...
  //  mui.modal.close('popup-apply');
	...
	function chg_insta(vno,vtype)
	{

if (typeof(vtype) == "undefined")		vtype ="";

	//	alert(vno);
//console.log(vno+" vinsta_url "+vinsta_url+" vtype "+vtype);

		$(".insta-list-tag a").removeClass("is-active");
		$(".insta-list-tag a").eq(vno).addClass("is-active");

		$("#detail-event2e-insta a").removeClass("is-active");
		$("#detail-event2e-insta a").eq(vno).addClass("is-active");

	//	console.log($(".insta-list-tag2 a").eq(vno));
		...
		if(chk_i9 =="Y")	{
			$('#tag_v02'+vtype).attr('src', './ista/t_ifr0.php?q='+vno);
		}
		else{
	//	('#tag_v02').attr('src', './ista/t_ifr1.php?q='+vno);
		...
		//	$('#tag_v02').attr('src', './ista/t_ifr1.php?q='+vno);
			$('#tag_v02'+vtype).attr('src', './ista/'+vinsta_url+'?q='+vno);
		}
 ...
 _trackga("Event2","인스타그램리스트2-"+filterStr);
	...
	function chg_insta_(vno)
	{
		...
		if(chk_i9 =="Y")	{
			...
			//$('#tag_v02').attr('src', './ista/t_ifr0.php?q='+vno);
			$('#tag_v02').attr('src', './ista/'+vinsta_url+'?q='+vno);
...
function chkevt2(ctype)
{
//	vinsta_url ="t_ifr1_em.php";
//	alert("AA");
	if(ctype =="A")
	{	
		vinsta_url ="t_ifr1.php";
	chg_insta('0');
	...
	vinsta_url ="t_ifr1_em.php";
	chg_insta('7','e');
//	document.getElementById("tag_v02e").contentDocument.location.reload(true);
	}
}
function viewevt2()
{
	...
	$(".event2-signpost-purchase").trigger( "click" );
}
...
function text_chg(rtype)
{
	if (rtype =="B")
	{
		$(".frm-text-detail").html("#잇츠스킨_엘모방문");
	}
	else
	{
		$(".frm-text-detail").html("#잇츠스킨_세서미스트리트 #세서미스트리트 #잇츠세서미 #잇츠스킨콜라보    ");
	}
