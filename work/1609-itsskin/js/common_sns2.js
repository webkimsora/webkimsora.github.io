...
function sendsns(stype)
{
		...
		link ="http://10thparty.itsskin.com/";
...
function goTwitter(title, link) {
	var title2 = "잇츠스킨 10th birthday party에 세서미 스트리트와 10센치가 놀러왔어요!"
...
//kakaostory//
    ...
    var INAPP = navigator.userAgent.match(/FB/i);
    

	
	var UserAgent2 = navigator.userAgent;
	var UKS = UserAgent2.match(/KAKAOSTORY/);
	
//	//alert(UKS);
//    if( UKS != null){
//		alert("A8");
//		var image ="http://10thparty.itsskin.com/images/2itsskin_sns_img_v1.jpg";
//		var obj = {method:'feed',display:'iframe',link:link,picture:image,name:title,description:title2};
...
////			display: 'iframe',
////            href: link
////        }, function(response){history.back(-1);});
//
//    } else 
//		
...
function goKakaoStory(title, link)
{
 ...
 var UserAgent2 = navigator.userAgent;
var UKS = UserAgent.match(/KAKAOSTORY/);
...
//    
    if( INAPP === null){
      var ab = window.open(href, 'fb222', 'width=600,height=650,left='+w+',top='+h+',scrollbars=0');
      if(ab) { ab.focus(); }
    } else {
      //window.location.href = href;
