...
$(function(){
   ...
   */
   ...
   $(document).ready(function(){       
	...
	// 181108 추가
	var movieArray = ['o4--epeL2hQ', '-WRu1Kmta2k', 'G8x5rG2CRyU']
	
	
	$(window).load(function(){
		//$('.movieArea #totalMovie').empty();
		//$('.movieArea #totalMovie').append('<iframe width="560" height="315" src="https://www.youtube.com/embed/' + movieArray[0] + '?rel=0&amp;autoplay=0&amp;showinfo=0" frameborder="0" allow="encrypted-media" allowfullscreen></iframe>')
		$('.movieTab li a').eq(1).trigger('click');
	})
	
	// 영상 탭
	$('.movieTab ul li a').on('click',function(){
		var idx = $(this).parent().index();
		if(idx == 0){
			$('.movieArea').removeClass('no2');
			$('.movieArea').removeClass('no3');
		} else if(idx == 1){
			$('.movieArea').addClass('no2');
			$('.movieArea').removeClass('no3');
		} else {
			$('.movieArea').addClass('no3');
			$('.movieArea').removeClass('no2');
		}
		
		$('.movieArea #totalMovie').empty();
		$('.movieArea #totalMovie').append('<iframe width="560" height="315" src="https://www.youtube.com/embed/' + movieArray[idx] + '?rel=0&amp;autoplay=0&amp;showinfo=0" frameborder="0" allow="encrypted-media" allowfullscreen></iframe>')
		
		//$('.movieArea #totalMovie iframe').attr('src','https://www.youtube.com/embed/' + movieArray[idx] + '?rel=0&amp;autoplay=0&amp;showinfo=0')
