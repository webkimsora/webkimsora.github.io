(function(){
  ...
  $(document).ready(function(){
    ...
    if($('.frame-play-contents .list li').hasClass('open')) {
      $(this).closest('.list-continue').hide();
    }
  });
...
//console.log(old_vidx +"__"+index);
    // 이전버트 
     //   $('.mv'+old_vidx).attr('src', './images/cnt-play/play-list'+old_vidx+'.png');
        $('.list-item:eq('+(old_vidx-1)+')').attr('src', './images/cnt-play/play-list'+old_vidx+'.png');

    $_self.closest('ul').find('button').removeClass('is-active');
    $_self.addClass('is-active');

    if($('.list button').hasClass('is-active')){
      $(this).children().attr('src', './images/cnt-play/play-list'+index+'-on.png');
    } else {
      $('.list-item').attr('src', './images/cnt-play/play-list'+index+'.png');
    }
  ...
  $(document).ready(function(){
    $('#wing-list').find('li').eq(0).find('a').trigger('click');
    isAutoPlay = 1;
  });
  ...
  $('#wing-list [data-video]').on('click', function(){
    ...
    //$('.wing-list-item:eq('+(old_vidx-1)+')').attr('src', './images/wing/wing'+old_vidx+'.png');

    $_self.closest('div').find('a').removeClass('clicked');
    $_self.addClass('clicked');

/*
    if($('.wing-list a').hasClass('is-active')){
      $(this).children().attr('src', './images/wing/wing'+index+'-on.png');
    } else {
      $('.wing-list-item').attr('src', './images/wing/wing'+index+'.png');
    }
