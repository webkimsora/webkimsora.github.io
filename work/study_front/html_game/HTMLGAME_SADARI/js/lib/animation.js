...
ani.ablelist='left|right|top|bottom|width|minWidth|height|minHeight|fontSize|color|letterSpacing|'+
	'textIndent|lineHeight|verticalAlign|margin|marginTop|marginRight|marginBottom|marginLeft|padding|'+
	...
	'borderBottomWidth|borderLeft|borderLeftColor|borderLeftWidth|borderRadius|background|'+
...
ani.split=function(v){
	...
	if(v[0].match(/(margin|padding|borderWidth|borderColor)/)){
		...
		var order=[[0,0,0,0],[0,1,0,1],[0,1,2,1],[0,1,2,3]];
...
ani.arraycomp=function(v1,v2){
	...
	for(var i=0,max=v1.length; i<max; i++) if(v1[i]!=v2[i]) return true;
	return false;
}

ani.getvs=function(fps,time,from,to,easing,isopacity){
	var v,rate,gap=to-from,rv=[];
...
ani.set=function(vs){
	...
	tg.timer=setTimeout(function(){
		...
		if(tg.onstart) tg.onstart.call(tg);
		ani.action(tg);
...
ani.action=function(tg){
	...
	if(Math.round(tg.time*tg.fps)>tg.step){
		...
		if(tg.onupdate) tg.onupdate.call(tg);
		tg.timer=setTimeout(function(){ani.action(tg)},(tg.time*1000)/(tg.fps*tg.time));
	}else{
		if(tg.step>Math.round(tg.time*tg.fps)-1) ani.setstyle(tg,Math.round(tg.time*tg.fps)-1);
		if(tg.onend) tg.onend.call(tg);
	}
}
