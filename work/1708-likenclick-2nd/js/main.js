...
//171103
//팝업 열기 ex) popAlert('CLASS or ID 값 넣기'); 
function popAlert(idx){
    $(idx).stop().fadeIn(100);
    //scrollDisabled();
}

//팝업 닫기 ex) popAlertClose('CLASS or ID 값 넣기'); 
function popAlertClose(idx){
    //scrollUpdate()
    $(idx).stop().fadeOut(100);
}
...
// 170817
	...
	$('.btnRullStart').on('click', function(){		
        popAlertClose('.benefitIntro');

        $('.everyday .rullDummy').hide();
        $('.everyday .rullWrap').show();
    ...
    // 171130 해피꿀
    $('.happyhoney .ballList > li').on('click',function(){
        $(this).addClass('active').siblings('li').removeClass('active');
    })

    $('.btnDonation').on('click', function(){
        $('.happyhoney .cntInWrap > div').eq(1).show().siblings('div').hide();
    })
