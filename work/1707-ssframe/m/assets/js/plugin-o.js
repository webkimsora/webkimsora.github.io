  ...
  mui.modal = (function(mui, $, undefined) {
   ...
   /*     @ 170317 김소라 수정 : px작업시 스크롤생길때 이슈가 생김 
      ...
      if( isOpenedModal('open') ){
        var $_self = isOpenedModal('open');
              $_self.addClass('is-opened');
               //$_self.css('top',$(window).scrollTop);
    ...
    $('[data-toggle="modal"]').on('click', function(e) {
      ...
      //var btnParent = $(this).parents('.section').offset().top;
      var btnParent = $(this).offset().top-300;

      alert($(window.parent).scrollTop());


        $(targetId).find('.modal-content').css('top',btnParent);
        

      openModal(target);

          setTimeout(function(){
            //var targetWidth = $(targetId).find('.modal-content').width();
            //$(targetId).find('.modal-content').css({'margin-left':-(targetWidth/2),'left':50+'%'})
            $(targetId).find('.modal-content').addClass('open');
